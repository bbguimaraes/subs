...
#define CHECK_FILE(f, x) check_file(LOC(), (f), (x), 0)
#define CHECK_FILE_N(f, x, n) check_file(LOC(), (f), (x), (n))
...
static inline bool check_file(
    struct loc loc, FILE *f, const char *s, size_t len
) {
    assert(fflush(f) != EOF);
    ...
    if(!log_len) {
        if(!len && !*s)
            return true;
        print_loc(loc);
        fprintf(stderr, "unexpected empty file\n");
        return false;
    }
    assert(fseek(f, 0, SEEK_SET) >= 0);
    char *text = checked_malloc((size_t)log_len);
    if(!text)
        return false;
    ...
    if(!ret) {
        print_loc(loc);
        fprintf(stderr, "unexpected file content: %.*s", (int)log_len, text);
    }
    free(text);
    return ret;
}
