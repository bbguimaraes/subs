...
bool update_youtube_init(struct update_youtube *u) {
    *u = (struct update_youtube){
    ...
    return exec_with_pipes(
        ...
        && exec_with_pipes(
            "python", (const char*[]){"python", "-uc", VIDEO_TIMESTAMP, NULL},
            &u->timestamp_pid, &u->timestamp_r, &u->timestamp_w);
}
...
bool update_youtube_destroy(struct update_youtube *p) {
    ...
    if(p->channel_w != -1 && close(p->channel_w) == -1)
        LOG_ERRNO("close", 0), ret = false;
    if(p->timestamp_r != -1 && close(p->timestamp_r) == -1)
        LOG_ERRNO("close", 0), ret = false;
    if(p->timestamp_w != -1 && close(p->timestamp_w) == -1)
        LOG_ERRNO("close", 0), ret = false;
    if(p->channel_pid != -1)
        ret = wait_for_pid(p->channel_pid) && ret;
    if(p->timestamp_pid != -1)
        ret = wait_for_pid(p->timestamp_pid) && ret;
    return ret;
}
...
static bool get_timestamp(
    ...
    const char *id, size_t id_len, struct buffer *b, i64 *p);
static bool insert(
...
static bool process_line(
    ...
    const char *title, size_t title_len, bool *done, int *n)
{
    const char sql[] = "select 1 from videos where yt_id == ?";
    bool exists;
    if(!exists_query(
    ...
    *done = false;
    i64 timestamp;
    if(!get_timestamp(u, ext_id, ext_id_len, b, &timestamp))
        return false;
    if(!timestamp)
        return true;
    return insert(
    ...
    const char *id, size_t id_len, struct buffer *b, i64 *p)
{
    ...
    if(write(u->timestamp_w, b->p, (size_t)n) != n)
        return LOG_ERRNO("write", 0), false;
    b->n = 0;
    buffer_reserve(b, 64);
    n = read(u->timestamp_r, b->p, b->cap - 1);
    if(n <= 0)
        return false;
    ((char*)b->p)[n] = 0;
    const i64 i = parse_i64((const char*)b->p);
    if(i == -1)
        return false;
    *p = i;
    return true;
...
{
    const char sql[] =
        "insert into videos (sub, yt_id, timestamp, title) values (?, ?, ?, ?)";
    sqlite3_stmt *stmt;
    ...
    if(verbose)
        fprintf(
            stderr, "created new video: %" PRId64 " %d %.*s %zu %.*s\n",
