...
static struct window *window_curses_new_window(int h, int w, int y, int x) {
    struct window_curses *const ret = malloc(sizeof(*ret));
    *ret = window_curses_new(newwin(h, w, y, x));
    return &ret->w;
}
...
{
    struct window_curses *const ret = malloc(sizeof(*ret));
    *ret = window_curses_new(derwin(window_curses_handle(pw), h, w, y, x));
    return &ret->w;
}
