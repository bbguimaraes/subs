...
static char *videos_row_to_str(sqlite3_stmt *stmt, int id) {
    ...
    assert(duration_seconds < 10 * 60 * 60 - 1);
    sprintf(
        duration_str, "%u:%02u:%02u",
...
static enum subs_curses_key input(struct videos *v, int c) {
    ...
    case '/':
        ...
        break;
    case 'H':
        list_move(l, l->offset);
        render_border(l, v);
        break;
    case 'L':
        list_move(l, l->offset + l->height - 3);
        render_border(l, v);
        break;
    case 'M':
        list_move(l, l->offset + (l->height - 2) / 2 - 1);
        render_border(l, v);
        break;
    case 'N':
        if(!l->n)
...
bool videos_reload(struct videos *v) {
    ...
    if(~v->flags & VIDEOS_ACTIVE)
        return true;
    struct reload_data *const d = malloc(sizeof(*d));
    if(!d)
    ...
    *d = (struct reload_data) {
    ...
    if(!task_thread_send(v->task_thread, (struct task){.f = reload, .p = d}))
        goto err;
    return true;
err:
    free(d);
