...
static bool select(struct window *w) {
    const struct source_bar *const b = w->data;
...
static void update_title(struct list *l, u8 flags) {
...
bool source_bar_leave(struct window *w) {
    list_set_active(&((struct source_bar*)w->data)->list, false);
    return true;
}

bool source_bar_enter(struct window *w) {
    list_set_active(&((struct source_bar*)w->data)->list, true);
    curs_set(0);
    return true;
}

void source_bar_redraw(struct window *w) {
    struct list *const l = &((struct source_bar*)w->data)->list;
    list_redraw(l);
    list_refresh(l);
}

enum subs_curses_key source_bar_input(struct window *w, int c) {
